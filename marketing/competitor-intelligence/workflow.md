...
| Tool | Purpose |
|------|---------|
| `marketing/competitor-intelligence/tools/scrape_competitor_ads.py` | Generate MCP instructions, compare snapshots, build intelligence reports |
...
Run `marketing/competitor-intelligence/tools/scrape_competitor_ads.py`:

```
python marketing/competitor-intelligence/tools/scrape_competitor_ads.py \
    --brand_category all \
...
This uses `save_snapshot()` from `marketing/competitor-intelligence/tools/scrape_competitor_ads.py`. The snapshot contains all fetched ad data plus metadata (date, tool version, ad counts).
