...
| `marketing/google-gmb/keywords_{brand_id}_auto_additions.json` | Auto-generated | Previous auto-additions (for merge/deduplication) |
...
| Tool | Purpose |
|------|---------|
| `marketing/seo-optimisation/tools/gsc_quick_win_finder.py` | Analyse GSC data, categorise queries, generate auto-additions |
...
4. Check `marketing/google-gmb/keywords_{brand_id}_auto_additions.json` for each brand (previous auto-additions)
...
1. Run `marketing/seo-optimisation/tools/gsc_quick_win_finder.py` analysis functions:
...
1. Call `load_existing_keywords(brand_id)` to get all existing keywords as a flat set
   ...
   - Includes JSON keyword banks from marketing/google-gmb/
...
2. Call `save_auto_additions(additions, brand_id)`:
   - Saves to `marketing/google-gmb/keywords_{brand_id}_auto_additions.json`
...
**Integration point:** These auto-additions are consumed by `marketing/google-gmb/tools/gbp_generate_posts.py` via the `merge_auto_additions()` function during the next GBP posting cycle.
...
| Auto-additions | `marketing/google-gmb/keywords_{brand}_auto_additions.json` | New keywords for GBP posting rotation |
...
- Auto-addition files at `marketing/google-gmb/keywords_{brand}_auto_additions.json` are consumed by `marketing/google-gmb/tools/gbp_generate_posts.py`
...
- **Implemented via:** launchd plist (`marketing/seo-optimisation/scheduling/com.carisma.gsc-hunter.plist`) that runs daily at 9am
