...
| Tool | Purpose |
|------|---------|
| `marketing/seo-optimisation/tools/wix_meta_optimiser.py` | Generate agent instructions, filter candidates, build MCP call parameters |
...
Run `marketing/seo-optimisation/tools/wix_meta_optimiser.py` to generate MCP instructions:

```
python marketing/seo-optimisation/tools/wix_meta_optimiser.py \
    --brand_id all \
    --days 30 \
    --max_pages 10 \
...
- **Time:** 10:00am
- **Mechanism:** launchd plist runs `marketing/seo-optimisation/tools/wix_meta_optimiser.py` daily at 10am, but the script checks `datetime.now().day == 1` and exits early if it is not the 1st
- **Override:** Set environment variable `WIX_SEO_FORCE_RUN=1` to bypass the monthly gate
- **Plist:** `marketing/seo-optimisation/scheduling/com.carisma.wix-seo.plist`
