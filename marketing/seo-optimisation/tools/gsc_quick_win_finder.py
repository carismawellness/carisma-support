...
Usage:
    python marketing/seo-optimisation/tools/gsc_quick_win_finder.py --brand_id all --days 28
    python marketing/seo-optimisation/tools/gsc_quick_win_finder.py --brand_id carisma_spa --days 14
    python marketing/seo-optimisation/tools/gsc_quick_win_finder.py --brand_id all  # (via launchd — exits early if not 1st/15th)
...
Outputs:
    ...
    Auto-addition files at marketing/google-gmb/keywords_{brand}_auto_additions.json
...
def save_auto_additions(
    ...
    """
    GBP_DIR.mkdir(parents=True, exist_ok=True)
    output_path = GBP_DIR / f"keywords_{brand_id}_auto_additions.json"
