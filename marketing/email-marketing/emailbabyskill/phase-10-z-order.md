...
8. Decorative elements (petals, badges, shapes, gold dividers)
...
**Pre-header bar (SLIM only):** `PreHeader_Bar_BG` → layer 2 (BG), `PreHeader_Bar_Text` → layer 6 (text). Both must be at y=0 (topmost visually, but z-order layered like other BG/text).
