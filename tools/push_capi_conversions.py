...
def build_capi_event(
    deal: dict[str, Any], brand: dict[str, Any], event_name: str
) -> dict[str, Any]:
...
def query_unsent_deals(
    ...
    try:
        ...
        log.info("COQL returned %d deals at stage '%s'", len(deals), trigger_stage)
        return deals
    except requests.HTTPError as e:
        log.warning("COQL query failed (%s), falling back to search", e)

    # Fallback: use search with criteria
    criteria = (
        f"((Stage:equals:{trigger_stage})"
        f"and(CAPI_Sent:equals:false))"
    )
    deals = zoho.search_records("Deals", criteria, DEAL_FIELDS)
    log.info("Search returned %d deals at stage '%s'", len(deals), trigger_stage)
    return deals
...
def mark_deals_sent(
    ...
    """Mark deals as sent in Zoho CRM."""
    ...
    # Zoho update API accepts max 100 records per call
    for i in range(0, len(records), 100):
        batch = records[i : i + 100]
        result = zoho.update_records("Deals", batch)
        log.info("Marked %d deals as CAPI_Sent=true: %s", len(batch), result)
...
def run_brand(brand_id: str, config: dict, dry_run: bool = False) -> dict[str, Any]:
    ...
    for event_key, event_cfg in capi_events_config.items():
        ...
        try:
            deals = query_unsent_deals(zoho, trigger_stage)
        ...
        if not dry_run and total_received > 0:
            ...
            try:
                mark_deals_sent(zoho, deal_ids, event_name)
            except Exception as e:
                log.error(
...
def run_all(dry_run: bool = False) -> list[dict[str, Any]]:
    ...
    for brand_id in ZOHO_ENV_PREFIXES:
        result = run_brand(brand_id, config, dry_run=dry_run)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.all_brands:
        ...
        config = load_brands_config()
        results = [run_brand(args.brand, config, dry_run=args.dry_run)]
