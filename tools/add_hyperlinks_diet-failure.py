#!/usr/bin/env python3
"""
add_hyperlinks_diet-failure.py

Applies real hyperlinks to the "Why Every Diet Has Failed You" Google Doc.
Based on add_docs_hyperlinks.py — updated DOCS_CONFIG for this specific post.

Usage:
    python tools/add_hyperlinks_diet-failure.py
"""
...
DOCS_CONFIG = [
    {
        "doc_id": "1pIa21ZHC5YJELI0iOhDSogx-0oa0dPbSjVhCkUZp3zA",
        "title": "SLI-03 — Why Every Diet Has Failed You",
        "final_md": f"{BASE_DIR}/FINAL-03-why-every-diet-failed-you.md",
    },
]
...
def find_link_occurrences(flat_text, index_map, anchor, url):
    ...
    # Also try simplified pattern without the arrow
    search_re2 = re.compile(escaped_anchor + r"\s*\[(?:→|->)\s*" + escaped_url + r"\]")

    results = []
    for pattern in [search_re2]:
...
def main():
    print("=" * 60)
    print("Carisma Blog — Add Hyperlinks: Why Every Diet Has Failed You")
    print("=" * 60)
    ...
    print(f"Doc URL: https://docs.google.com/document/d/1pIa21ZHC5YJELI0iOhDSogx-0oa0dPbSjVhCkUZp3zA/edit")
