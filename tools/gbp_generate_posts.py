...
Usage:
    python tools/gbp_generate_posts.py --brand_id all --num_posts 2
    python tools/gbp_generate_posts.py --brand_id carisma_spa --post_type offer --num_posts 1
...
def load_locations_config() -> dict[str, Any]:
    ...
    config_path = GBP_CONFIG_DIR / "locations.json"
    if not config_path.exists():
...
def load_content_calendar() -> dict[str, Any]:
    ...
    config_path = GBP_CONFIG_DIR / "content-calendar.json"
    if not config_path.exists():
...
def load_keyword_bank(brand_id: str) -> dict[str, list[str]]:
    ...
    # Try Markdown
    md_path = GBP_CONFIG_DIR / f"keywords_{brand_id}.md"
    if md_path.exists():
...
def load_templates(brand_id: str) -> dict[str, list[dict[str, Any]]]:
    ...
    templates_path = GBP_CONFIG_DIR / f"templates_{brand_id}.json"
