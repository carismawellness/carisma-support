#!/usr/bin/env python3
"""
add_hyperlinks_emssculpt.py

Applies real clickable hyperlinks to the EMSculpt NEO Malta blog post Google Doc.
Converts 'anchor text [-> URL]' placeholders to real hyperlinks via Google Docs API.

Usage:
    python tools/add_hyperlinks_emssculpt.py
"""
...
BASE_DIR = "/Users/mertgulen/Library/CloudStorage/GoogleDrive-mertgulen98@gmail.com/My Drive/Carisma Wellness Group/Carisma AI /Carisma AI/.tmp/blog-drafts/slimming/july-2026"

DOCS_CONFIG = [
    {
        "doc_id": "1GGTxnIwTMQUcgxasupv77GvtZRbmlrUEz210XxYoB5c",
        "title": "SLI-02 (July 2026) — EMSculpt NEO Malta",
        "final_md": f"{BASE_DIR}/FINAL-02-emssculpt-neo-malta.md",
    },
]
...
def find_link_occurrences(flat_text, index_map, anchor, url):
    ...
    # Also handle the Unicode arrow
    search_re2 = re.compile(escaped_anchor + r"\s*\[(?:\u2192|->)\s*" + escaped_url + r"\]")

    results = []
    for search in [search_re, search_re2]:
        for m in search.finditer(flat_text):
            full_start = m.start()
...
def main():
    ...
    print("Post: EMSculpt NEO Malta (July 2026)")
    print("=" * 60)
    ...
    print(f"Doc URL: https://docs.google.com/document/d/1GGTxnIwTMQUcgxasupv77GvtZRbmlrUEz210XxYoB5c/edit")
