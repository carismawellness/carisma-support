#!/usr/bin/env python3
"""
add_hyperlinks_glp1.py

Converts 'anchor text [-> URL]' visible link placeholders in the GLP-1 Google Doc
to real clickable hyperlinks using the Google Docs API batchUpdate.

Post: GLP-1 Weight Loss Malta: What You Need to Know Before Starting
...
DOCS_CONFIG = [
    {
        "doc_id": "1XfyzcMvrTZH4IpbVS1iJA5GgGdgjHnkfrfdw7ZISxW8",
        "title": "SLI-01 — GLP-1 Weight Loss Malta",
        "final_md": f"{BASE_DIR}/FINAL-01-glp1-weight-loss-malta.md",
    },
]
...
def find_link_occurrences(flat_text, index_map, anchor, url):
    ...
    # Also try the simpler pattern that matches the actual format written in the doc
    search_re2 = re.compile(escaped_anchor + r"\s*\[" + r"(?:\u2192|->)" + r"\s*" + escaped_url + r"\]")

    results = []
    for regex in [search_re, search_re2]:
        for m in regex.finditer(flat_text):
            full_start = m.start()
...
def main():
    ...
    print("Post: GLP-1 Weight Loss Malta")
    print("=" * 60)
    ...
    print(f"Doc URL: https://docs.google.com/document/d/1XfyzcMvrTZH4IpbVS1iJA5GgGdgjHnkfrfdw7ZISxW8/edit")
