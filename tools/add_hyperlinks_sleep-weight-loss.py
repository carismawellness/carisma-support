#!/usr/bin/env python3
"""
add_hyperlinks_sleep-weight-loss.py

Converts 'anchor text [→ URL]' visible link placeholders in the Google Doc
for the Carisma Slimming post: "Sleep and Weight Loss Malta — September 2026"
to real clickable hyperlinks using the Google Docs API batchUpdate.

Usage:
    python3 tools/add_hyperlinks_sleep-weight-loss.py [DOC_ID]

If DOC_ID is not passed as an argument, uses the default configured below.
Reads FINAL.md to extract (anchor_text, url) pairs, then finds each
occurrence in the Google Doc and applies links via batchUpdate.
...
SLI_DIR = "/Users/mertgulen/Library/CloudStorage/GoogleDrive-mertgulen98@gmail.com/My Drive/Carisma Wellness Group/Carisma AI /Carisma AI/.tmp/blog-drafts/slimming/september-2026"

DOCS_CONFIG = [
    {
        "doc_id": "1WY6-yWtfwoMI3KRnZYpjcQBVNUvD-Wj_WgWFGhTNwMI",
        "title": "SLI-SEP-01 — Sleep and Weight Loss Malta",
        "final_md": f"{SLI_DIR}/post-01-sleep-weight-loss-FINAL.md",
    },
]
...
def main():
    # Allow optional doc_id override via CLI argument
    if len(sys.argv) > 1:
        override_id = sys.argv[1]
        for cfg in DOCS_CONFIG:
            cfg["doc_id"] = override_id
        print(f"Using doc ID from argument: {override_id}")

    print("=" * 60)
    print("Carisma Blog — Sleep & Weight Loss — Add Hyperlinks")
    print("=" * 60)
    ...
    print(f"Doc URL: https://docs.google.com/document/d/{DOCS_CONFIG[0]['doc_id']}/edit")
