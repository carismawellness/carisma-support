...
- `config/gbp/keywords_{brand}_auto_additions.json` -- Previous auto-additions (if they exist)
...
New keywords discovered by the GSC Hunter are written to `config/gbp/keywords_{brand}_auto_additions.json`. The GBP Post Generator (`tools/gbp_generate_posts.py`) automatically picks up these files during keyword bank loading via the `merge_auto_additions()` function.
...
4. Keyword is added to `config/gbp/keywords_carisma_spa_auto_additions.json` with category "primary"
