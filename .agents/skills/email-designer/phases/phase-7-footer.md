...
**Prerequisite:** Phase 6 (Colours)
**References:** golden-rules.md (rules 3, 9, 14), brands/<brand>.md § Grid Node IDs
**Next:** Phase 8 (Waves / Dividers)
...
The footer node ID and expected height are brand-specific — see `brands/<brand>.md` § Grid Node IDs → Row 5: Fixed Footer for the exact footer ID and height.

## 7.2 Place Footer

1. `mcp__figma-write__clone_node(nodeId: "<footer_id>")` — where `<footer_id>` comes from `brands/<brand>.md` § Grid Node IDs → Row 5: Fixed Footer
...
1. "#1 MOST REVIEWED SPA IN MALTA" text
2. Five gradient-filled star shapes
3. "OVER 3,000+ FIVE-STAR REVIEWS" text
4. "THE CARISMA DIFFERENCE" heading
...
1. "REAL RESULTS" heading + five gold stars
2. Before/After photo thumbnails (2-4 images, 130x160px, 8px radius)
3. "THE CARISMA DIFFERENCE" + 3 circular photos (70x70px)
...
2. 4 trust circles (56x56px) — verify count = 4
3. **"EXPLORE CARISMA SLIMMING" navigation section** with stacked pill buttons:
   - Weight Loss | Body Sculpting | Skin Tightening | GLP-1
   - Pill specs: see `brands/slim.md` § Decorative Elements → Nav Pills for background color and corner radius
...
5. Dark green footer block (see `brands/slim.md` § Color Palette for footer BG color):
