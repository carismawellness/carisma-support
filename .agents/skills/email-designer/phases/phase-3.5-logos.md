...
**Prerequisite:** Phase 3 — CTA Buttons
**References:** golden-rules.md (rules 9, 11, 14), brands/<brand>.md § Grid Node IDs
**Next:** Phase 4 — Spacing & Overlap Detection

---

> **Reminder (Golden Rule #14):** `<production_frame>` below means the EXISTING Row 7 grid frame — see `brands/<brand>.md § Grid Node IDs` → Row 7 for the correct node IDs. Never a self-created frame.
...
Look up logo node IDs from `brands/<brand>.md § Grid Node IDs` → Row 1: Logos.

**SPA and AES — known IDs:**
- Read the Logo Frame node ID from `brands/<brand>.md § Grid Node IDs` → Row 1
- `get_node_info` on that ID → inspect children for brand colour + white variants
...
| Brand colour logo | Hero (if hero is not too dark) or pre-footer | Brand colour on light background. Use when hero gradient allows visibility. |
