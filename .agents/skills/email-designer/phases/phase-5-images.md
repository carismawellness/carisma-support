...
**Prerequisite:** Phase 4 (Spacing & Overlap)
**References:** golden-rules.md (rules 8, 9, 11, 12), brands/<brand>.md § Image Filters, § Grid Node IDs
**Next:** Phase 5.5 (Nano Banana — if fallback needed), otherwise Phase 6 (Colours)
...
7. **Apply resolution tiers:**
   ...
   - **Hero images:** target >= 2.0x (source >= 1200x600 for 600x300 display). Accept >= 1.5x if no better option. Never accept < 1.0x.
...
d. **Orientation-aware transform** — the source aspect ratio determines the correct scale and offset:
   ...
   |-------------------|-------------|-------|------------|-----|
   | **Landscape** (W > H) | e.g., 1200x800 | 1.2 | 0 to -0.1 | Width fills circle. Subject usually centered. Minor Y adjust for framing. |
   | **Square** (W ~ H) | e.g., 800x800 | 1.2 | 0 | Perfect fit. Slight zoom for tighter crop. |
   | **Portrait** (H > W) | e.g., 800x1200 | 1.0 | -0.15 to -0.25 | Width fills circle but height is cropped heavily. Pull UP (negative translateY) to show face, not torso. |
   | **Tall portrait** (H > 1.5xW) | e.g., 600x1200 | 0.9-1.0 | -0.2 to -0.35 | Very tall crop. May need to zoom OUT slightly (scale < 1.0) to show more of the subject, then offset to face. |
...
- Fill: see `brands/<brand>.md` § Color Palette for placeholder gray
- Thin stroke: see `brands/<brand>.md` § Color Palette for stroke color at 40% opacity
- Labels: "BEFORE" and "AFTER" — see `brands/<brand>.md` § Typography for label font
...
**Filter values are brand-specific** — see `brands/<brand>.md` § Image Filters for the exact temperature, saturation, contrast, and exposure values.
...
Where X, Y, Z, W come from `brands/<brand>.md` § Image Filters.

**Do NOT over-filter.** These are subtle brand-tone adjustments, not Instagram presets. Values in the brand config should be treated as starting points — adjust by +/-0.05 if the result looks unnatural.
...
2. **Check for these problems:**
   - **Orange/yellow cast** (warm brands) -> reduce temperature to +0.05 or 0
   - **Over-saturated / neon tones** (vibrant brands) -> reduce saturation to +0.05 or 0
   - **Washed out / too bright** (clinical brands) -> reduce exposure to 0
...
4. **Log filter decisions** in the state file for reproducibility:
   ...
   |-------|----------------|-----------|-------|
   | Hero | [from brand config] | No | Looked natural |
