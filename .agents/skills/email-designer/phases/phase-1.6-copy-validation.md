...
**Prerequisite:** phase-1.5-image-discovery.md (Image Inventory complete)
**References:** golden-rules.md (rules 1, 9, 10, 14), brands/<brand>.md (Grid Node IDs)
**Next:** phase-2-text-hierarchy.md
...
1. Re-read the Row 6 wireframe frame using `get_node_info`. Source IDs: see `brands/<brand>.md` § Grid Node IDs (Row 6: Copy + Wireframe).
...
1. `get_node_info` on BOTH Row 6 frames for the brand (see `brands/<brand>.md` § Grid Node IDs for both emailer 1 + emailer 2 frame IDs)
...
1. `get_node_info` on the Row 7 production frame (see `brands/<brand>.md` § Grid Node IDs, Row 7: Final Production)
