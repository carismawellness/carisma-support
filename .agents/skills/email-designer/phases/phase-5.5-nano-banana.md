...
**Prerequisite:** Phase 5 (Images) — triggered when Phase 1.5 flagged sections with no Image Bank match
**References:** golden-rules.md (rules 8, 12), brands/<brand>.md § Identity, § Image Filters
**Next:** Phase 6 (Colours)
...
**Brand modifiers** are defined in `brands/<brand>.md` § Identity — use the brand's tone, atmosphere, and visual language to construct the modifier. The modifier should convey the brand's setting, color palette feel, and mood. For example:
- A warm luxury spa brand -> warm golden tones, luxury spa atmosphere, serene and peaceful mood, Mediterranean architecture
- A clean clinical aesthetics brand -> clean clinical elegance, modern minimalist aesthetic, bright airy space, professional medical atmosphere
- A vibrant wellness/slimming brand -> vibrant healthy lifestyle, empowering and confident mood, active wellness setting, natural light

Consult `brands/<brand>.md` § Color Palette and § Identity to derive the correct modifier for the active brand.
...
- Style matches brand aesthetic (compare against `brands/<brand>.md` § Identity for expected mood)
...
Run Phase 5.3 brand image filters on the generated image. Filter values: see `brands/<brand>.md` § Image Filters.
...
- Same brand modifiers apply (derived from `brands/<brand>.md` § Identity)
...
|---------|--------|-------------|---------|--------|
| Hero | nano-banana | "Professional [brand] wellness..." | [id] | generated |
| Section 3 | nano-banana | "Close-up [treatment]..." | [id] | generated |
```
